...
maxy = 2500
eyesight = 2
typeSet = ['terminator','arrow','data','process','decision','text']
typeDict = {'terminator':0,'arrow':1,'data':2,'process':3,'decision':4,'text':5}
    ...
    for tg in tGlist:
        ...
        for tgc in tg.childNodes:
            ...
            if tgc.nodeName == 'traceView':
                tmptrl.append(tgc.getAttribute('traceDataRef'))
        if elementType in typeSet:
            if elementType == 'text':
                traceRefList = traceRefList + tmptrl
    #print(len(tracelist))
    for trace in tracelist:
        tid = trace.getAttribute('id')
        if tid in traceRefList:
            continue
...
def get_category(namestr):
    ctgry = {}
    ctgry['id'] = typeDict[namestr]
    ctgry['name'] = namestr
...
def get_flowchart(filename, image_map, img_id):
    ...
    for tg in tGlist:
        ...
        if elementType in typeSet:
            print("current element type is {}".format(typeDict[elementType]))
            annotation['id'] = curid
            annotation['image_id'] = img_id
            annotation['category_id'] = typeDict[elementType]
            ...
            categories = get_category(elementType)
...
for fp in pathlist:
    ...
    annstr = get_flowchart(filepath, image_map, cnt)
    #image_list = image_list + img_str + ','
    image_list.append(img_str)
    ann_list= ann_list + annstr
    cnt = cnt + 1

for tpname in typeSet:
    ctgrystr = get_category(tpname)
    ctgry_list.append(ctgrystr)
